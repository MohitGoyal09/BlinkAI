...
export interface HarnessState {
  ...
  // True from when user sends message until agent_start event received
...
export function useHarness() {
  ...
  const handleEvent = useCallback((event: HarnessEvent & { threadId?: string }, isReplay = false) => {
    ...
    // Cast to any to allow extended event types not in HarnessEvent union
    switch ((event as any).type) {
      ...
      case 'info':
        patch({ infoMessage: (event as any).message })
        break

      case 'error':
        patch({ error: (event as any).error, waitingForAgent: false })
        break
      ...
      // --- Messages ---
      case 'message_start':
        patch({ currentStreamingMessage: (event as any).message })
        break
      ...
      case 'message_end': {
        ...
        const idx = existing.findIndex(m => m.id === (event as any).message.id)
        const updated = idx >= 0
          ? existing.map((m, i) => (i === idx ? (event as any).message : m))
          : [...existing, (event as any).message]
      ...
      case 'tool_start': {
        ...
        ts.set((event as any).toolCallId, {
      ...
      case 'tool_approval_required': {
        const ts = new Map(s.toolStates)
        const prev = ts.get((event as any).toolCallId)
        ts.set((event as any).toolCallId, {
          toolCallId: (event as any).toolCallId,
          toolName: (event as any).toolName,
          args: (event as any).args,
        ...
        patch({
          toolStates: ts,
          pendingToolApproval: { toolCallId: (event as any).toolCallId, toolName: (event as any).toolName, args: (event as any).args },
        })
        break
      }
      ...
      case 'tool_update': {
        ...
        const prev = ts.get((event as any).toolCallId)
        if (prev) {
          ts.set((event as any).toolCallId, { ...prev, partialResult: (event as any).partialResult })
          patch({ toolStates: ts })
        }
        break
      }
      ...
      case 'tool_end': {
        ...
        ts.set((event as any).toolCallId, {
          ...
          args: prev?.args,
          status: (event as any).isError ? 'error' : 'completed',
          result: (event as any).result,
          isError: (event as any).isError,
          ...prev && { shellOutput: prev.shellOutput },
        })
        // Clear pending approval if this was the approved tool
        const pendingApproval = s.pendingToolApproval?.toolCallId === (event as any).toolCallId ? null : s.pendingToolApproval
      ...
      case 'shell_output': {
        ...
        if (prev) {
          ts.set((event as any).toolCallId, {
            ...prev,
            shellOutput: (prev.shellOutput ?? '') + (event as any).output,
          })
          patch({ toolStates: ts })
        }
        break
      ...
      case 'subagent_start': {
        ...
        ss.set((event as any).toolCallId, {
          toolCallId: (event as any).toolCallId,
      ...
      case 'subagent_text_delta': {
        const ss = new Map(s.subagentStates)
        const prev = ss.get((event as any).toolCallId)
        if (prev) {
          ss.set((event as any).toolCallId, { ...prev, text: prev.text + (event as any).textDelta })
          patch({ subagentStates: ss })
        }
        break
      }
      ...
      case 'subagent_tool_start': {
        ...
        if (prev) {
          const tool: SubagentToolState = {
            toolName: (event as any).subToolName,
            args: (event as any).subToolArgs,
            status: 'running',
          }
          ss.set((event as any).toolCallId, { ...prev, nestedTools: [...prev.nestedTools, tool] })
          patch({ subagentStates: ss })
        }
        break
      }
      ...
      case 'subagent_tool_end': {
        ...
        if (prev) {
          const tools = prev.nestedTools.map(t =>
            t.toolName === (event as any).subToolName && t.status === 'running'
              ? { ...t, result: (event as any).subToolResult, isError: (event as any).isError, status: (event as any).isError ? 'error' as const : 'completed' as const }
              : t
          )
          ss.set((event as any).toolCallId, { ...prev, nestedTools: tools })
          patch({ subagentStates: ss })
        }
        break
      }
      ...
      // --- Mode / Model ---
      case 'mode_changed':
        patch({ currentModeId: (event as any).modeId })
        break

      case 'model_changed':
        patch({ currentModelId: (event as any).modelId })
        break
      ...
      case 'ask_question':
        patch({
          pendingQuestion: {
            questionId: (event as any).questionId,
      ...
      // --- Usage ---
      case 'usage_update':
        patch({ tokenUsage: (event as any).usage })
        break

      // --- Follow-up ---
      case 'follow_up_queued':
        patch({ followUpCount: (event as any).count })
        break
      ...
      // --- OM ---
      case 'om_status':
        patch({ omStatus: event as HarnessEvent & { type: 'om_status' } })
        break
  ...
  const sendMessage = useCallback(async (content: string, images?: { data: string; mimeType: string }[]) => {
    ...
    patch({ messages: [...stateRef.current.messages, userMsg], waitingForAgent: true })
