...
export async function seedBuiltinSkills() {
  ...
  for (const name of fs.readdirSync(builtinDir)) {
    ...
    try {
      ...
      if (err?.code === 'EACCES' || err?.code === 'EPERM') {
        console.warn(`[seedBuiltinSkills] Permission denied accessing skill: ${src} — skipping`);
        failed++;
      } else {
        console.warn(`[seedBuiltinSkills] Failed to seed skill ${name}: ${err?.message}`);
        failed++;
      }
    }
  }
  ...
  for (const skill of fs.readdirSync(skillsDir)) {
    ...
    try {
      ...
      if (err?.code === 'EACCES' || err?.code === 'EPERM') {
        console.warn(`[seedBuiltinSkills] Permission denied reading scripts directory: ${scriptsDir} — skipping`);
      }
      continue;
    }
    ...
    for (const script of scripts) {
      ...
      try {
        ...
        if (err?.code === 'EACCES' || err?.code === 'EPERM') {
          console.warn(`[seedBuiltinSkills] Permission denied accessing script: ${scriptPath} — skipping`);
        }
      }
    }
  }
  ...
  if (failed > 0) {
    console.log(`[skills] failed to seed ${failed} skill(s) due to permission errors`);
  }
}
