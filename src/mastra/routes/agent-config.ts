    ...
    registerApiRoute('/agent-config', {
      ...
      handler: async (c) => {
        ...
        if (body.yoloMode !== undefined) {
          if (body.yoloMode === null) {
            agentConfig.delete('yolo_mode');
          } else {
            agentConfig.set('yolo_mode', body.yoloMode ? 'true' : 'false');
          }
          import('../harness/pool').then(pool => pool.harnessPool.updateYoloMode(!!body.yoloMode));
        }
        const config = await agentConfig.getConfig();
        return c.json(config);
      },
